...
for level_num in [args.level]: # 1, 2
    ...
    for OT_name in ep_map[level_num].keys():
        ...
        if level_num == 1:
            ...
            current_policy = policies.Hierarchical_Policies(level_num, OT_name, folder_name = f'full_run_logs/saved_models_lvl{level_num}_seed{args.seed}_{args.type}_{args.gamma}_{args.learn_freq}_{args.lr}_{args.mem_size}_{args.n_layers}_{args.n_hidden_units}', use_demos = False, gamma = args.gamma, learn_freq = args.learn_freq, critic_lr = args.lr, mem_size = args.mem_size, n_layers = args.n_layers, n_hidden_units = args.n_hidden_units)
