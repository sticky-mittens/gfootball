...
class Hierarchical_Policies():
    ...
    def update_policy(self, env, obs, obs_prev, done, shaped_reward_function, learnt_options_set, ep_num, debug):
        ...
        obs_wrapped = utils.obs_wrapper(obs, self.level_num) # (1, 139) 2D list
        ...
        self.total_action_OT_calls_while_learning_for_lvl1 += 1
        ...
        while True:
            if utils.all_OTs[self.level_num+1][action_OT_name]['pre'](obs): 
                ...
                if self.level_num == 1:
                    obs, obs_prev, reward, done, info, self.ct = teleports.do_nothing(env, obs, obs_prev, done, self.ct)
                    reward = 0
                break
        
        shaped_reward = shaped_reward_function(obs, obs_prev, action_OT_name, reward)

        if goal_scored and self.level_num == 2:
        ...
        self.memory.push(ep_num, (self.FloatTensor(obs_wrapped_pre_teleport).to(self.device), # (1, 139)
                ...
                self.FloatTensor(utils.obs_wrapper(obs, self.level_num)).to(self.device), # (1, 139)
    ...
    def LearnGuidedPolicy(self, env, shaped_reward_function, learnt_options_set, ep_num, debug = False):
        ...
        self.total_action_OT_calls_while_learning_for_lvl1 = 0
        self.total_calls_lvl2 = 0
        self.correct_calls_lvl2 = 0
        ...
        while not done:
            ...
            if top_option_name == self.OT_name:
                ...
                obs_wrapped = utils.obs_wrapper(obs, top_option_level)
                obs_wrapped_tensor = self.FloatTensor(obs_wrapped).to(self.device)
                ...
                if action_OT_name in utils.all_OTs[self.level_num+1]:
                    ...
                    if utils.all_OTs[self.level_num+1][action_OT_name]['post'](obs): 
                        if debug: print(f'{self.level_num}) {action_OT_name} terminated and popped')
                        execution_stack.pop() 
                else:
                    self.total_calls_lvl2 += 1
                    if utils.all_OTs[top_option_level+1][action_OT_name]['pre'](obs):
                        ...
                        self.correct_calls_lvl2 += 1
                    else: # New (1): in case win_game network makes a mistake and predicts attack when you have to defend (or vice versa)
                        obs, obs_prev, reward, done, info, self.ct = teleports.do_nothing(env, obs, obs_prev, done, self.ct)
        ...
        if self.level_num == 2:
            ...
            heapq.heappush(self.memory.ep_scorediff_tuples_heap, (sum_reward/self.total_action_OT_calls_while_learning_for_lvl1, ep_num))
        ...
        if self.level_num == 1:
            print(f'=== ep: {ep_num}, time {time.time()-start_time_ep}, eps {self.eps_threshold}, right preds for atk and def: {sum_reward}/{self.total_action_OT_calls_while_learning_for_lvl1} = {sum_reward/self.total_action_OT_calls_while_learning_for_lvl1}, score_diff {current_score_diff}, tot learning steps {self.t} (total env steps {self.ct})')
        elif self.level_num == 2:
            print(f'=== ep: {ep_num}, time {time.time()-start_time_ep}, eps {self.eps_threshold}, sum reward: {sum_reward}, score_diff {current_score_diff}, tot learning steps {self.t} (total env steps {self.ct}) (win_game called correctly {self.correct_calls_lvl2}/{self.total_calls_lvl2})')
    ...
    def save_to_disk_extra_lvl1(self, R, n_action_OTs, ep_num):
        os.makedirs(f'./{self.folder_name}', exist_ok = True)
        torch.save(self.critic, f'./{self.folder_name}/{self.OT_name}_{R}_{n_action_OTs}_{R/n_action_OTs}_{ep_num}.pt')
