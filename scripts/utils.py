...
def get_env_with_saved_videos(render, type, seed):
    N_PLAYERS_LEFT = 11
    N_PLAYERS_RIGHT = 0
    ...
    os.makedirs(f'videos_while_solving_{type}_seed{seed}', exist_ok=True)
    ...
    env = football_env.create_environment(
        ...
        write_full_episode_dumps=True,
        write_video=True,
        logdir=f'videos_while_solving_{type}_seed{seed}/',
        number_of_left_players_agent_controls=N_PLAYERS_LEFT,
        number_of_right_players_agent_controls=N_PLAYERS_RIGHT,
        render=render,
        other_config_options={#'physics_steps_per_frame': 3, 
...
def obs_wrapper(obs_input, level_num):
    '''
